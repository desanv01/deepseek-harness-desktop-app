...
| 2. Unit tests | **partly done** | Two behavioural suites run in CI: `tools/smoke-test.ps1` (CLI discovery, repair, install safety, the plugin pipeline, the newer-plugin rule, safe-mode recovery, the served boot graph, and the page bridge's message shapes - 63 assertions) and `tools/client-plugin-smoke.mjs`, which renders the updates plugin's browser half under a minimal React and a fake shell that enforces the real slot rules, so its UI is checked where WebView2 cannot start (53 assertions). The pure-logic unit tests below are still open |
