...
function Get-PnpmRuntime {
    <#
      The pnpm runtime the app carries for plugin installs. tools\fetch-pnpm.ps1
      owns the download (CI uses the same script); this only calls it, and a
      failure is not fatal because the app fetches the runtime on demand.
    #>
    param([switch]$Skip)
    if ($Skip) { Write-Output 'skipping the pnpm runtime (-SkipPnpm)'; return }
    & pwsh -NoProfile -File (Join-Path $root 'tools\fetch-pnpm.ps1')
}
